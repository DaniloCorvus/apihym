...
class FlujoEfectivoController extends Controller
{
    ...
    public function  getInfo()
    {
        ...
        $this->fecha_fin =  Carbon::now()->endOfDay();
        $this->fecha_inicio = Carbon::now()->startOfDay();


        $funcionarios = [];

        if (request()->has('Fecha_fin') && request()->get('Fecha_fin') != '' && request()->get('Fecha_fin') != 'null' && request()->get('Fecha_fin') != 'undefined') {

            $this->fecha_fin = Carbon::parse(request()->get('Fecha_fin'))->addDay();
        }

        if (request()->has('Fecha_inicio') && request()->get('Fecha_inicio') != '' && request()->get('Fecha_inicio') != 'null' && request()->get('Fecha_inicio') != 'undefined') {
    ...
    function getFuncionarios($oficina)
    {
        ...
        return  DB::select(
            "SELECT fun.Identificacion_Funcionario FROM Diario as dia
                                                        ...
                                                        INNER JOIN Oficina as offff On offff.Id_Oficina = dia.Oficina_Apertura
                                                        WHERE offff.Id_Oficina = $oficina
